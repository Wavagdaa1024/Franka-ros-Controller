...
class FrankaActionExecutor:
    def __init__(
        ...
        self._gripper_trigger_count = 4   # 连续3次才触发


        # ===== 夹爪动作冷却时间锁 =====
        self._gripper_action_lock_sec = 3.0
        self._gripper_last_action_time = 0.0
    ...
    def apply_gripper_command(self, gripper_cmd: float):
        ...
        # ===== 3秒时间锁：刚执行过开/关，先不允许反向改变 =====
        if now - self._gripper_last_action_time < self._gripper_action_lock_sec:
            return
        ...
        # 连续达到触发次数才真正闭合
        if self._gripper_close_count >= self._gripper_trigger_count:
            if not self._gripper_closed:
                self.arm.close_gripper()
                self._gripper_closed = True
                self._gripper_last_action_time = now
                rospy.loginfo(f"Cloud action gripper: close (cmd={float(gripper_cmd):.3f})")
            self._gripper_close_count = 0
            self._gripper_open_count = 0

        # 连续达到触发次数才真正张开
        elif self._gripper_open_count >= self._gripper_trigger_count:
            if self._gripper_closed:
                self.arm.open_gripper()
                self._gripper_closed = False
                self._gripper_last_action_time = now
                rospy.loginfo(f"Cloud action gripper: open (cmd={float(gripper_cmd):.3f})")
            self._gripper_close_count = 0
            self._gripper_open_count = 0
    ...
    def execute_single_action(self, action: np.ndarray, running_check=None, active_check=None):
        ...
        '''
        # ===== z 下限保护，防止撞传送带 =====
        if target_pos[2] < 0.075:
            rospy.logwarn(f"Clamp z from {target_pos[2]:.4f} to 0.1000")
            target_pos[2] = 0.075
        '''
...
class CloudFrankaInferenceClient:
    def __init__(
        ...
        self.simulated_latency_sec = float(simulated_latency_sec)
        self.stitch_overlap_steps = stitch_overlap_steps
        ...
        # ===== 网络延迟估计与动作截断 =====
        self._latency_warmup_count = 10          # 前10次只预热，不统计
        self._latency_sample_target = 10         # 再取后10次做平均
        self._latency_warmup_done = 0
        self._latency_samples = []
        self._avg_latency_sec = 0.0
        self._action_trim_steps = 0
        self._latency_ready = False
    ...
    def _trim_actions_by_latency(self, actions: np.ndarray) -> np.ndarray:
        actions = np.asarray(actions, dtype=np.float32)

        if actions is None or len(actions) == 0:
            return actions

        trim_steps = int(self._action_trim_steps)
        self._action_trim_steps = trim_steps

        if trim_steps <= 0:
            return actions

        if trim_steps >= len(actions):
            print(
                f"[WARN] trim_steps={trim_steps} >= actions_len={len(actions)}, "
                f"drop this action chunk."
            )
            return np.empty((0, actions.shape[1]), dtype=np.float32)

        print(
            f"[INFO] avg_latency={self._avg_latency_sec:.3f}s, "
            f"action_dt={self.executor.action_dt:.3f}s, "
            f"trim first {trim_steps} actions."
        )
        return actions[trim_steps:].copy()
    ...
    def _stitch_pending_plan(self, new_actions: np.ndarray) -> np.ndarray:
        new_actions = np.asarray(new_actions, dtype=np.float32).copy()
        if len(new_actions) == 0:
            return new_actions

        # 当前没有 active plan，就退化成原来的轻平滑
        if self._active_plan is None or self._active_plan_idx >= len(self._active_plan):
            return self._smooth_new_plan(new_actions)

        remain_old = np.asarray(self._active_plan[self._active_plan_idx:], dtype=np.float32).copy()
        if len(remain_old) == 0:
            return self._smooth_new_plan(new_actions)

        k = min(self.stitch_overlap_steps, len(remain_old), len(new_actions))
        if k <= 0:
            return self._smooth_new_plan(new_actions)

        stitched = []

        # old plan 前面没执行完、且不参与 overlap 的部分先保留
        if len(remain_old) > k:
            stitched.extend(remain_old[:-k])

        # overlap blending
        for i in range(k):
            alpha = float(i + 1) / float(k + 1)

            old_a = remain_old[-k + i].copy()
            new_a = new_actions[i].copy()
            mixed = old_a.copy()

            # 只拼 xyz
            mixed[:3] = (1.0 - alpha) * old_a[:3] + alpha * new_a[:3]

            # gripper 不做连续插值，避免半开半关
            mixed[3] = old_a[3] if alpha < 0.5 else new_a[3]

            stitched.append(mixed)

        # 新 plan 剩余部分接上
        if len(new_actions) > k:
            stitched.extend(new_actions[k:])

        stitched = np.asarray(stitched, dtype=np.float32)

        # 再做一次头部轻平滑
        return self._smooth_new_plan(stitched)
    ...
    async def communication_loop(self, websocket):
        while not rospy.is_shutdown() and self._running:
            ...
            if self._latency_ready:
                effective_prefetch_threshold = max(self.prefetch_threshold, self._action_trim_steps)
            else:
                effective_prefetch_threshold = self.prefetch_threshold

            need_request = (
                self._active_plan is None or
                (
                    self._active_plan is not None and
                    len(self._active_plan) - self._active_plan_idx <= effective_prefetch_threshold
                )
            )
            ...
            try:
                ...
                request_latency = time.time() - t0
                self._sync_latency_samples.append(request_latency)

                avg_request_latency = float(np.mean(self._sync_latency_samples))
                print(
                    f"[SYNC LATENCY] step={self._step}, "
                    f"last={request_latency:.3f}s, avg={avg_request_latency:.3f}s"
)

                # 先预热10次，不参与统计
                if self._latency_warmup_done < self._latency_warmup_count:
                    self._latency_warmup_done += 1
                    print(
                        f"[INFO] latency warmup {self._latency_warmup_done}/{self._latency_warmup_count}: "
                        f"{request_latency:.3f}s"
                    )

                # 预热结束后，再采样10次做平均
                elif not self._latency_ready:
                    self._latency_samples.append(request_latency)

                    print(
                        f"[INFO] latency sample {len(self._latency_samples)}/{self._latency_sample_target}: "
                        f"{request_latency:.3f}s"
                    )

                    if len(self._latency_samples) >= self._latency_sample_target:
                        measured_latency_sec = float(np.mean(self._latency_samples))
                        self._avg_latency_sec = measured_latency_sec + self.simulated_latency_sec
                        self._action_trim_steps = int(np.ceil(self._avg_latency_sec / self.executor.action_dt))
                        self._latency_ready = True

                    print(
                        f"[INFO] latency ready: measured={measured_latency_sec:.3f}s, "
                        f"simulated={self.simulated_latency_sec:.3f}s, "
                        f"total={self._avg_latency_sec:.3f}s, "
                        f"trim_steps={self._action_trim_steps}"
                    )

                if actions is not None:
                    if not self._latency_ready:
                        print("[INFO] latency not ready, skip execution this round.")
                        await asyncio.sleep(0.01)
                        continue

                    actions = self._trim_actions_by_latency(actions)

                    if actions is not None and len(actions) > 0:
                        # self._pending_plan = self._stitch_pending_plan(actions)
                        self._pending_plan = self._smooth_new_plan(actions)
                    else:
                        print("[WARN] all actions trimmed away, skip this update.")
        ...
        while not rospy.is_shutdown() and self._running:
            ...
            if self._active_plan_idx >= len(self._active_plan):
                if self._pending_plan is not None:
                    ...
                    self._pending_plan = None
                    self._active_plan_idx = 0
                    print(f"[Step {self._step}] switched immediately to pending plan shape={self._active_plan.shape}")
                else:
                    self._active_plan = None
                    await asyncio.sleep(0.005)
                    continue
            ...
            print(f"[TIME] step={self._step}, elapsed={elapsed:.3f}s")
            ...
            async with websockets.connect(self.server_uri, max_size=None) as websocket:
                while not rospy.is_shutdown() and self._running:
                    ...
                    try:
                        ...
                        actions = self._smooth_new_plan(actions)
                        ...
                        # print(f"[TOTAL TIME] step={self._step}, elapsed={elapsed:.3f}s")

                        elapsed = time.time() - self._sync_start_time
                        print(f"[SYNC TIME] step={self._step}, elapsed={elapsed:.3f}s")
