...
def publisher_callback(msg, link_name):
    ...
    pose_pub.publish(marker_pose)     # 把目标位置不断发送给机器人的核心动作

# 当你在 RViz 里用鼠标拖动标记时，这个函数就会被触发
def process_feedback(feedback):
    ...
    server.applyChanges()  # 处理完限制后，更新 marker_pose 的坐标和姿态（orientation），并应用更改


# 监听franka_state_controller/franka_states，获取实际位姿，设定为交互标记的初始位置
...
if __name__ == "__main__":
    rospy.init_node("equilibrium_pose_node")    # 这个名字会被覆盖，由launch文件中的node标签的name属性指定
    listener = tf.TransformListener()
    link_name = rospy.get_param("~link_name")

    wait_for_initial_pose()  # 等待机器人状态消息，获取初始位姿，并设置交互标记的初始位置
    
    # # 创建广播员（发布者），频道名是"equilibrium_pose"，格式是PoseStamped，缓存10条
    ...
    # 创建 6 个拖动控制柄，分别对应 x、y、z 轴的旋转和移动
